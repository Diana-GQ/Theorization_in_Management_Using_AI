...
from constants import ATTRIBUTES_FILE, TWITTER_COLUMNS
...
def cleanup(x: str) -> List:
    ...
    tokens = [
        (t.text, t.lemma_)
        for t in nlp(" ".join(tokenizer.tokenize(x)))
        if t.lemma_ not in stopwords
        and t.lemma_ not in punctuation
        and t.lemma_ not in misc_blocked
    ]
    return tokens
...
def load_posts(input_data_path: str) -> pd.DataFrame:
    ...
    for f in csv_profiles:
        ...
        df_tmp = pd.read_csv(f, usecols=TWITTER_COLUMNS, parse_dates=["created_at"])
